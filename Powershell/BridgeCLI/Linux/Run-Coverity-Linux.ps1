<#
.SYNOPSIS
    Executes Coverity scan using Bridge CLI on Linux
...
.EXAMPLE
    pwsh ./Run-Coverity-Linux.ps1 -CoverityUrl "https://coverity.example.com" -CoverityUser "admin" -CoverityPassword "password" -ProjectName "MyApp" -StreamName "MyApp-main"
#>
...
Write-Host "Coverity Scan - Linux" -ForegroundColor Cyan
Write-Host "========================================" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    # Test Coverity connectivity using curl
    Write-Host "Testing Coverity Connect connectivity..." -ForegroundColor Yellow
    try {
        $curlTest = curl -I -s -m 30 $CoverityUrl 2>&1
