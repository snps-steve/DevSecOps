<#
.SYNOPSIS
    Downloads and installs Bridge CLI on Linux for Azure DevOps pipelines
...
.DESCRIPTION
    ...
    Works with PowerShell Core on Linux.
...
if ($BridgeVersion -eq "latest") {
    $DownloadUrl = "$BridgeBaseUrl/latest/bridge-cli-linux64.zip"
} else {
    $DownloadUrl = "$BridgeBaseUrl/$BridgeVersion/bridge-cli-linux64.zip"
}

Write-Host "========================================" -ForegroundColor Cyan
Write-Host "Bridge CLI Installation Script - Linux" -ForegroundColor Cyan
Write-Host "========================================" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    # Download Bridge CLI
    $ZipPath = Join-Path -Path $InstallDirectory -ChildPath "bridge-cli-linux64.zip"
    Write-Host "Downloading Bridge CLI..." -ForegroundColor Yellow
    
    # Use curl for download (more reliable on Linux)
    $curlArgs = @(
        '-fLsS',
        '-o', $ZipPath,
        $DownloadUrl
    ...
    $curlProcess = Start-Process -FilePath 'curl' -ArgumentList $curlArgs -Wait -NoNewWindow -PassThru
    
    if ($curlProcess.ExitCode -ne 0) {
        throw "Failed to download Bridge CLI. Exit code: $($curlProcess.ExitCode)"
    }
    
    Write-Host "Download completed successfully!" -ForegroundColor Green
    Write-Host ""
    
    # Extract Bridge CLI using unzip
    ...
    $unzipProcess = Start-Process -FilePath 'unzip' -ArgumentList $unzipArgs -Wait -NoNewWindow -PassThru
    
    if ($unzipProcess.ExitCode -ne 0) {
        throw "Failed to extract Bridge CLI. Exit code: $($unzipProcess.ExitCode)"
    }
    ...
    # Verify installation and set executable permissions
    $BridgeExePath = Join-Path -Path $InstallDirectory -ChildPath "bridge-cli-bundle-linux64/bridge-cli"
    
    if (Test-Path -Path $BridgeExePath) {
        # Make executable
        chmod +x $BridgeExePath
