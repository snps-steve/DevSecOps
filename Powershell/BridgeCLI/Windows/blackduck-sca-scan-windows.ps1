<#
.SYNOPSIS
    Executes Black Duck SCA scan using Bridge CLI on Windows
...
.EXAMPLE
    .\Run-BlackDuckSCA-Windows.ps1 -BlackDuckUrl "https://blackduck.example.com" -BlackDuckToken "your-token" -ProjectName "MyApp" -ProjectVersion "main"
#>
...
Write-Host "Black Duck SCA Scan - Windows" -ForegroundColor Cyan
Write-Host "========================================" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    try {
        $response = Invoke-WebRequest -Uri $BlackDuckUrl -Method Head -TimeoutSec 30 -UseBasicParsing -ErrorAction SilentlyContinue
